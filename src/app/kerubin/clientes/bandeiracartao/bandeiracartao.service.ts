...
export class BandeiraCartaoService {
	...
	url = environment.apiUrl + '/kerubin/clientes/entities/bandeiraCartao';
	...
	bandeiraCartaoNomeAutoComplete(query: string): Promise<any> {
	    const headers = this.getHeaders();
	
	    let params = new HttpParams();
	    params = params.set('query', query);
	
	    return this.http.get<any>(`${this.url}/bandeiraCartaoNomeAutoComplete`, { headers, params })
	      .toPromise()
	      .then(response => {
	        const result = response as BandeiraCartaoNomeAutoComplete[];
	        return result;
	      });

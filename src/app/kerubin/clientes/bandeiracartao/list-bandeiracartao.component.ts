...
import { BandeiraCartaoNomeAutoComplete } from './bandeiracartao.model';
...
export class BandeiraCartaoListComponent implements OnInit {
	...
	bandeiraCartaoNomeAutoCompleteSuggestions: BandeiraCartaoNomeAutoComplete[];
	...
	*/
	...
	ngOnInit() {
    	this.bandeiraCartaoListFilter.sortField = new SortField('nome', 1); // asc
	...
	bandeiraCartaoNomeAutoComplete(event) {
	    const query = event.query;
	    this.bandeiraCartaoService.bandeiraCartaoNomeAutoComplete(query)
	    .then((result) => {
	      this.bandeiraCartaoNomeAutoCompleteSuggestions = result;
	    })
	    .catch(erro => {
